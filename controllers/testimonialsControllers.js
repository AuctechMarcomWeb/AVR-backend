...
const createTestimonial = asyncHandler(async (req, res) => {
  try {
    const { title, description, rating, profileImage, isActive } = req.body;
    ...
    if (rating !== undefined && (rating < 0 || rating > 5)) {
      return res
        .status(422)
        .json(new apiResponse(422, null, "Rating must be between 0 and 5"));
    }

    // 🔹 Duplicate Check (based on title + description OR profileImage)
      ...
      return res
        ...
        .json(
          new apiResponse(
            409,
            null,
            "Duplicate resource: testimonial already exists with similar data",
          ),
        );
    }
